...
class RadialSampling():
    ...
    def get_kdata_ktraj(self, df, mag_readout_start, prob_map, n_readout, t2, t_readout, reverse_readout=False):
        """
        ...
        for i in range(total_num):
            ...
            for j, mag in enumerate(mag_readout_start):
                ...
                if reverse_readout:
                    ky = self.ky[gp[j], ::-1].copy()
                    kx = self.kx[gp[j], ::-1].copy()
                else:
                    ky = self.ky[gp[j], :]
                    kx = self.kx[gp[j], :]
    ...
    def visual_logKdata(self, kdata):
        kdata_numpy = np.reshape(kdata.cpu().numpy(), (self.nspokes, self.spokelength))
        plt.imshow(np.log10(np.abs(kdata_numpy) + 1e-12))
        plt.gray()
